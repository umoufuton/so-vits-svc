...
def load_model_func(ckpt_name,cluster_name,config_name,enhance,diff_model_name,diff_config_name,only_diffusion,use_spk_mix,using_device,method,speedup):
    global model
    config_path = os.path.join(config_read_dir, config_name)
...
def refresh_options():
    global ckpt_read_dir, config_read_dir, diff_read_dir
...
def vc_infer(output_format, sid, input_audio, input_audio_path, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment):
    ...
    cluster = "" if cluster_ratio == 0 else f"_{cluster_ratio}"
    isdiffusion = "sovits"
    if model.shallow_diffusion : isdiffusion = "sovdiff"
    if model.only_diffusion : isdiffusion = "diff"
    output_file_name = 'result_'+Path(input_audio_path).stem[:-6]+f'_{sid}_{key}_{cluster}_{isdiffusion}.{output_format}'
...
def vc_batch_fn(output_format, sid, input_audio_files, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment):
    ...
    try:
        ...
        for file_obj in input_audio_files:
            ...
            vc_infer(output_format, sid, audio, input_audio_path, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num, lgr_num, f0_predictor, enhancer_adaptive_key, cr_threshold, k_step, use_spk_mix, second_encoding, loudness_envelope_adjustment)
...
def dataset_preprocess(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes):
    if precheck_ok:
        ...
        #if vol_aug and encoder == "vec768l12":
        #    yield accumulated_output, gr.Textbox.update(value=spk_name), None
        #else:
        #    yield accumulated_output, gr.Textbox.update(value=spk_name), None
...
def config_fn(log_interval, eval_interval, keep_ckpts, batch_size, lr, amp_dtype, all_in_mem, diff_num_workers, diff_cache_all_data, diff_batch_size, diff_lr, diff_interval_log, diff_interval_val, diff_cache_device, diff_amp_dtype, diff_force_save, diff_k_step_max):
    fp16_run = True if amp_dtype == "fp16" or "bf16" else False
    ...
    #if tiny_enable:
    #    config_data['model']['filter_channel'] = 512
    #    config_data['model']['upsample_initial_channel'] = 320
    #    config_data['model']['n_flow_layer'] = 3
    #    config_data['model']['flow_share_parameter'] = True
...
def release_packing(model_to_pack, model_config, speaker, diff_to_pack, cluster_to_pack):
    ...
    shutil.copyfile("configs_template/config_template.json", f"release_packs/config_template.json")
    shutil.copyfile("configs_template/diffusion_template.yaml", f"release_packs/diffusion_template.yaml")
    files_to_pack = [
...
with app:
    gr.Markdown(value="""
        ### So-VITS-SVC 4.1-Stable WebUI 推理&训练 v2.3.7
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("训练"):
            ...
            with gr.Row():
                ...
                f0_predictor_selection = gr.Dropdown(label="选择训练使用的f0预测器", choices=["crepe","pm","dio","harvest"], value="crepe", interactive=True)
            with gr.Row():
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    amp_dtype = gr.Radio(label="训练数据类型，fp16可能会有更快的训练速度和更低的显存占用，但容易炸炉", choices=["fp32","fp16","bf16"], value=sovits_params['amp_dtype'])
